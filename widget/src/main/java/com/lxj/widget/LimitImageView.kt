...
class LimitImageView @JvmOverloads constructor(
    ...
    var imgHeight = 0
    init {
        limitWidth = WidgetUtil.dp2px(160f)
        limitHeight = WidgetUtil.dp2px(160f)
    }
    ...
    fun setImageSize(w: Int, h: Int){
        ...
        if(ratio < 1){
            ...
            if(realW!=0 && height!=0){
                WidgetUtil.widthAndHeight(this, realW, height)
            }
        }else{
            //按宽度等比缩放
            ...
            if(width!=0 && realH!=0){
                WidgetUtil.widthAndHeight(this, width, realH)
            }
        }
    }
}