        ...
        try {
            ...
            while ((length = in.read(bytes)) != -1) {
                ...
                if (mIsCanceled || read >= PreloadManager.PRELOAD_LENGTH) {
                    if (mIsCanceled) {
                        LogUtils.d("预加载取消：" + mRawUrl + " 读取数据：" + read + " Byte");
                    } else {
                        LogUtils.d("预加载成功：" + mRawUrl + " 读取数据：" + read + " Byte");
                    }
                    break;
                }
            }
        } catch (Exception e) {
            LogUtils.d("预加载异常：" + mRawUrl + " 异常信息："+ e.getMessage());
            // 关入小黑屋
