...
public class WebProgress extends FrameLayout {
     ...
     * 进度条颜色默认
     */
    public static String WEB_PROGRESS_COLOR = "#2483D9"; //2483D9
    /**
     * 进度条颜色
     */
    private int mColor;
...
//        mTargetHeight = dip2px(WEB_PROGRESS_DEFAULT_HEIGHT);
    ...
    public void setColor(int startColor, int endColor) {
        LinearGradient linearGradient = new LinearGradient(0, 0, getMeasuredWidth(), getMeasuredHeight(), startColor, endColor, Shader.TileMode.CLAMP);
    ...
    protected void dispatchDraw(Canvas canvas) {
        canvas.drawRect(0, 0, mCurrentProgress / 100f * getWidth(), getHeight(), mPaint);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (getMeasuredWidth() >= screenWidth) {
            ...
            //取比值
            float rate = getMeasuredWidth() / Float.valueOf(screenWidth);
